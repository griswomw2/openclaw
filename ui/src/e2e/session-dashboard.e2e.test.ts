...
suite.define(() => {
  ...
  it("renders and updates active Workboard plugin widgets", async () => {
    ...
    try {
      ...
      // The visible chat header refreshes its catalog, while hidden widgets retain their paint.
      expect(await gateway.getRequests("workboard.cards.list")).toHaveLength(
        listCountBeforeHide + 1,
      );
      expect(await cardWidget.textContent()).toContain(readyCard.title);
      expect(await cardWidget.textContent()).not.toContain(refreshedCard.title);

      await mode("split").click();
      // Both widgets resume one shared read without remounting their native views.
      await expect
